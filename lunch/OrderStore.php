  	...
  	if ($row == NULL) {
		...
		$i=0;
  		while ($row != NULL) {
			...
			//$tpl->assign(storename,"<a target='_blank' href='/lunch/UsrPdsDetails.php?id=$row[StoreID]'>".$info[StoreName]."</a>");
			$tpl->assign(storename,$info[StoreName]);
			//$tpl->assign(order,"<a href='/lunch/OrderLunch.php?id=$row[StoreID]&mid=$row[RecordID]'>".$LnhG->ManagerStatus[$row[Status]]."</a>");
